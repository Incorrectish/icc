...
impl AsmGenerator {
        ...
        match function_declaration {
            ast::FuncDecl::Func(indentifier, statements) => {
                ...
                for statement in statements {
                    assembly.add_instructions(self.gen_block_item(statement, parent_scope));
                }
        ...
        match declaration {
            ast::Declaration::Declare(name, optional_expression, optional_child_declaration) => {
                let mut decleration_assembly = if let Some(expression) = optional_expression {
                    let location = self.symbol_table.allocate(name, parent_scope, LONG_SIZE, parent_scope);
    ...
    fn gen_block(&mut self, block: Vec<ast::BlockItem>) -> Asm {
        ...
        for block_item in block {
            asm.add_instructions(self.gen_block_item(block_item, parent_scope));
        }
        asm
    }
    ...
    fn gen_expression(&mut self, expr: ast::Expression, parent_scope: u64) -> Asm {
        match expr {
            ...
            ast::Expression::Assign(name, expression) => {
                ...
                let key = (name, parent_scope);
                let optional_location = self.symbol_table.get(&key);
                if let Some(location) = optional_location {
                    ...
                    fail!(
                        "exp(assign)\nVariable \"{}\" in scope '{} referenced but not declared", key.0, key.1
                    );
                }
            }
            ast::Expression::ReferenceVariable(name) => {
                ...
                if let Some(location) = optional_location {
                    ...
                    fail!("Variable \"{}\" in scope '{} referenced but not declared", key.0, key.1);
                }
            }
        ...
        match operator {
            ...
            ast::UnaryOperator::PostfixIncrement(name) => {
                ...
                let location = self
                    .symbol_table
                    .get(&key)
                    .expect(&format!("Variable {} in scope '{} accessed but not declared", key.0, key.1));
